  ...
  return (
    <>
      ...
      <footer style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'32px 48px', borderTop:`1px solid ${isFunky ? '#292524' : '#F5F5F4'}` }}>
        ...
        <div style={{ fontFamily:'var(--font-mono)', fontSize:10, letterSpacing:'0.08em', color:textLo }}>
          © 2026 · Built with Next.js + Tailwind · v2.0
        </div>
        <div style={{ display:'flex', gap:24 }}>
          {['Work','About','Contact'].map(l => (
            <a key={l} href={`#${l.toLowerCase()}`} style={{ fontFamily:'var(--font-mono)', fontSize:10, textTransform:'uppercase', letterSpacing:'0.06em', textDecoration:'none', color:textLo, transition:'color 0.15s' }}
            onMouseEnter={e => e.currentTarget.style.color = accent}
            onMouseLeave={e => e.currentTarget.style.color = textLo}>{l}</a>
          ))}
        </div>
      </footer>
    </>
